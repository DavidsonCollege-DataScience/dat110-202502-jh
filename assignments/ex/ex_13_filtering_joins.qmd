...
-   Learn how to use filtering joins like `semi_join()` and `anti_join()` to merge two tables or data frames together and filter their product based on attributes they have in common.
...
wb_rowcount <- worldbank |> count() |> mutate(source = "World Bank")
# Count of rows in WID data
wid_rowcount <- countries |> count() |> mutate(source = "World Inequality Database")
# Union to get results together
union_all(wb_rowcount, wid_rowcount)
```
...
lfratio <- read_delim("../../data/lfratio.csv")

```

Note that the country codes in our data are consistently 3-digit codes. However, as we look back at our data, we need to keep the following constraints in mind:
...
This is data from two different sources spread across four tables, but there is enough shared data between them that we can leverage joins to unify everything. It requires us to carefully traverse these tables, which is going to be a multi-step process with three joins, so let's go through this iteratively.

Use the below diagram as a guide. Just render your Quarto file to HTML to see the output!
...
Now let's try to write the pipeline by filling in the empty sections here:

```{r}
combined_df <- worldbank |>
  # first join
  
  # Select only the columns we need
  select(
    iso2c, iso3c, year = `year.y`, fmratio
  ) |> 
  # second join

  # third join

  # Rename from "value" to "flis"
  rename(flis = value) |>
  # Reorder the columns slightly
  relocate(c(year, fmratio, flis), .after = subregion)
...
combined_df |>
  ...
  labs(
    title    = "Relationship between female income share and workforce participation",
    subtitle = "Segmented by world region",
    x        = "Female-to-Male Workforce Participation Ratio",
    y        = "Female Labor Income Share (FLIS)",
    color    = "Region"
  )
```
